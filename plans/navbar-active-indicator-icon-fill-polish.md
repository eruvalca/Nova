...
- [x] `Nova.Browser.Tests/NavbarBrowserTests.cs`:
  ...
  - Add a mobile companion test (e.g. `Navbar_Mobile_ExpandedMenu_KeepsInlineRowAndOutlineGlyph`): in its own 480×800 context (per the reviewer's recommendation — a separate context avoids the collapse-toggling flakiness), expand the collapsed menu and assert each authorized link keeps `flex-direction: row`, the icon box sits beside (not above) the label box at the 1.25rem slot baseline, and an inactive link keeps the outline glyph visible with the `-fill` overlay hidden.
...
Tests: NB1–NB5 kept green; NB2 extended (flush + fill + outline-return assertions); NB6 added (stacked layout at 1280×800); NB7 added (mobile <md inline row + outline glyph at 480×800, per review remediation); bUnit NavMenu test asserts all five `-fill` glyph pairs in the markup. Full build/format/unit/browser validation passed locally.
