...
Manual `node .agents/skills/impeccable/scripts/detect.mjs` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `node .agents/skills/impeccable/scripts/detect.mjs --no-config ...` for a raw detector run that ignores project config/context. Use the local hook-admin.mjs commands below to manage the same detector ignores.
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `node .agents/skills/impeccable/scripts/detect.mjs <path>` first to see what actually fires there.
