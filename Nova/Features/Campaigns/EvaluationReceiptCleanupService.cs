...
/// <summary>Globally removes bounded batches of expired, aggregate-independent evaluation receipts.</summary>
...
internal sealed partial class EvaluationReceiptCleanupService(IServiceScopeFactory scopeFactory, ILogger<EvaluationReceiptCleanupService> logger) : BackgroundService
{
    /// <inheritdoc />
    ...
    internal static async Task PruneAsync(NovaAdminDbContext db, DateTimeOffset now, CancellationToken token)
    {
        ...
        {
            await db.EvaluationMutationReceipts.Where(receipt => receipt.RecoveryExpiresAt <= now)
                .OrderBy(receipt => receipt.RecoveryExpiresAt).ThenBy(receipt => receipt.EvaluationMutationReceiptId)
                .Take(500).ExecuteDeleteAsync(token);
        }
        else
        {
            var rows = await db.EvaluationMutationReceipts.ToListAsync(token);
            db.EvaluationMutationReceipts.RemoveRange(rows.Where(receipt => receipt.RecoveryExpiresAt <= now)
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "Evaluation receipt cleanup failed; the next hourly pass will retry.")]
