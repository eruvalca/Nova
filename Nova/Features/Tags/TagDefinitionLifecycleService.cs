    ...
    ILogger<TagDefinitionLifecycleService> logger)
{
    ...
    /// <summary>
    /// Applies one tag-definition lifecycle transition attempt inside a single transaction using a
    /// fresh tenant context.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            LogTagLifecycleCommitVerified(tagDefinitionId, targetStatus);
            return new ExecutionResult<OneOf<Success, NotFound, LifecycleForbidden, LifecycleConflict>>(
                successful: true,
                new Success());
        }
    ...
    /// <summary>
    /// Logs an ambiguous commit verified to have applied the requested lifecycle transition.
    /// </summary>
    /// <param name="tagDefinitionId">The tag-definition identifier whose transition was verified.</param>
    /// <param name="status">The applied lifecycle status.</param>
    [LoggerMessage(Level = LogLevel.Information, Message = "PlayerTagId={TagDefinitionId} lifecycle transition to {Status} verified after an ambiguous commit.")]
    private partial void LogTagLifecycleCommitVerified(long tagDefinitionId, LifecycleStatus status);
}
