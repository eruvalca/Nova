...
LEFT JOIN (
    ...
    WHERE (FALSE OR (p2."ClubId" = 2 AND (FALSE OR c3."Status" <> 2))) AND p2."ClubId" = 2 AND p4."ClubId" = 2 AND c3."ClubId" = 2 AND s2."ClubId" = 2 AND c3."SeasonId" = c4."CurrentSeasonId" AND c3."Status" IN (0, 1) AND c3."SeasonOpeningSequence" IS NOT NULL AND p2."PlacementOutcome" <> 0
) AS s5 ON s5."PlayerCampaignAssignmentId" = (SELECT p5."PlayerCampaignAssignmentId"
