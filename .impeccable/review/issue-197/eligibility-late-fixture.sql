...
FROM (
    ...
    LEFT JOIN (
        ...
        INNER JOIN (
            ...
            WHERE FALSE OR (c2."ClubId" = 22 AND (FALSE OR c2."Status" <> 2))
        ) AS c3 ON p2."CampaignId" = c3."CampaignId"
        INNER JOIN (
            SELECT p3."PlayerId", p3."ClubId"
            FROM "Players" AS p3
            WHERE FALSE OR p3."ClubId" = 22
        ) AS p4 ON p2."PlayerId" = p4."PlayerId"
        INNER JOIN (
            SELECT s1."SeasonId", s1."ClubId"
            FROM "Seasons" AS s1
            WHERE FALSE OR s1."ClubId" = 22
        ) AS s2 ON c3."SeasonId" = s2."SeasonId" AND c3."ClubId" = s2."ClubId"
        INNER JOIN "Clubs" AS c4 ON c3."ClubId" = c4."ClubId"
        WHERE (FALSE OR (p2."ClubId" = 22 AND (FALSE OR c3."Status" <> 2))) AND p2."ClubId" = 22 AND p4."ClubId" = 22 AND c3."ClubId" = 22 AND s2."ClubId" = 22 AND c3."SeasonId" = c4."CurrentSeasonId" AND c3."Status" IN (0, 1) AND c3."SeasonOpeningSequence" IS NOT NULL AND p2."PlacementOutcome" <> 0 AND NOT EXISTS (
            SELECT 1
