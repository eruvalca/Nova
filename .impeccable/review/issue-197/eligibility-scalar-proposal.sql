...
FROM (
    ...
    WHERE (FALSE OR (p."ClubId" = 2 AND (FALSE OR c0."Status" <> 2))) AND p."ClubId" = 2 AND p1."ClubId" = 2 AND c0."ClubId" = 2 AND s0."ClubId" = 2 AND c0."Status" = 0 AND c0."SeasonId" = c1."CurrentSeasonId" AND p."CampaignId" = 3
) AS s6
GROUP BY s6."Key";
