...
public partial class CampaignParticipantDrawer
{
    ...
    private async Task LoadNotesAsync(bool append)
    {
        ...
        {
            if (sequence == _notesSequence && string.Equals(owner, ContextOwner, StringComparison.Ordinal))
            {
                _notesError = "Notes could not be loaded.";
            }
        }
    }
    ...
    private async Task LoadApplicationsAsync(bool append)
    {
        ...
        {
            if (sequence == _applicationsSequence && string.Equals(owner, ContextOwner, StringComparison.Ordinal))
            {
                _applicationsError = "Applications could not be loaded.";
            }
        }
    }
}
