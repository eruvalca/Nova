...
- The uncommitted-departure guard warns on document unload, same-origin link departure and browser
  Back/Forward while the board holds typed input, released on commit, cancel and set-aside. The
  traversal is answered through the Navigation API's `navigate` event, which fires before the traversal
  commits and while the board is still mounted (thirty-eighth pass, below). The module observes DOM input
...
## GitHub Copilot code review, thirty-eighth pass (PR #285, on `8c2c1f46`, fixed in `7b9eea5a`)

This pass carries one inline thread and one body finding. The thread is the **fourth raise** of the departure
guard's Back/Forward gap, which rounds 5, 11 and 23 had each narrowed with measurements; the body finding is a
focus target that cannot receive focus.

| # | Finding | Disposition |
| --- | --- | --- |
| 1 | The departure contract is not met for browser Back/Forward: `beforeunload` and the click listener do not run for history traversal, so typed fields are lost without the prompt; the finding asks to keep a page-level history/navigation guard alive through the traversal, or otherwise restore/intercept the entry (`PlayerIntakeBoard.razor.js:161`, **inline thread**) | **Fixed — the gap is closed, not narrowed again.** The earlier rounds measured the two mechanisms that could not work (`popstate` is delivered after the router has taken the route, and a page-level `NavigationLock` is not consulted for a traversal), but **not** the Navigation API's own `navigate` event, which fires *before* a traversal commits and while the board is still mounted. A probe on the real build recorded it: the listener ran once for a `traverse` with the board connected, `cancelable` was true, and cancelling it left the URL on `/players/new` with the typed value intact. The module cancels the traversal and asks through the same receiver the click path uses, so consent navigates to the destination the traversal was headed for; a cross-document traversal is not cancelable there and stays with the unload prompt. `PlayerFormHistoryTraversalAsksBeforeDiscardingTypedInputAsync` types, is refused on both departure paths, keeps the value through **Keep editing**, and leaves on consent; with the listener removed the same case fails. The pinned limitation case it replaces is deleted, and the two current-state bullets below are corrected. |
| 2 | Server feedback applied while the recovery is `Unresolved` disables the whole fieldset, so the focus request for `.intake-fields .is-invalid` cannot move focus — browsers ignore focus on a disabled control — leaving the member on the submit control (`PlayerIntakeBoard.razor.cs:425`, **review body**) | **Fixed in the module, where focus is applied.** `focusRegion` hands focus to the region a disabled control names through its own `aria-describedby`, and keeps the control as the target whenever it can take focus; the board's request stays the semantic one it always was, so no transient state can mis-decide it. `IntakeBoardModuleFocusesTheDescribedRegionOfADisabledControlAsync` drives the module in the browser and requires `probe-error` while the fieldset is disabled and `probe-field` once it is enabled; without the fallback it fails. |

### Confirming evidence (Copilot thirty-eighth pass)

Tested revision: `7b9eea5a` (the record's own commit follows it).

| Check | Command / result |
| --- | --- |
| Build | `dotnet build Nova.slnx` — **passed, 0 warnings, 0 errors**. |
| Full unit | `dotnet test --project Nova.Unit.Tests/Nova.Unit.Tests.csproj --no-build` — **3884 total, 3884 passed, 0 failed, 0 skipped** (3883 before; one case added). |
| Negative checks | With the traversal listener removed, the **revert build re-verified as successful (0 warnings, 0 errors) before the run**, `PlayerFormHistoryTraversalAsksBeforeDiscardingTypedInputAsync` fails. With the module's focus fallback removed, the same build check then fails `IntakeBoardModuleFocusesTheDescribedRegionOfADisabledControlAsync`. Both restored, rebuilt and re-run green. |
| Format | `dotnet format Nova.slnx --verify-no-changes` — **exit 0**. |
| Full integration | `dotnet test --project Nova.Integration.Tests/Nova.Integration.Tests.csproj --no-build` — **678 total, 678 passed, 0 failed, 0 skipped**. |
...
- **Finish review complete; formal hero measurement still absent.** The independent review returned
  ...
  - A probe for the board's *own* dispatch → storage path never got there: the submit produced
    ...
    not as a failure.
  - **Browser Back/Forward is intercepted** (thirty-eighth pass, below): the Navigation API's `navigate`
    event fires before the traversal commits and while the board is still mounted, so cancelling it keeps
    the member on the form with their input. The #270 evaluation guard protects the same path with its own
    restore-and-replay dance because its prompt opens after the traversal lands; this board's listener
    never lets the traversal commit, so it needs none of that machinery.
...
- **The departure guard now intercepts browser Back/Forward**, through the Navigation API's `navigate`
  event (thirty-eighth pass). The gap rounds 5, 11 and 23 recorded is closed rather than narrowed: the
  event fires before a traversal commits and while the board is still mounted, so cancelling it keeps the
  member on the form with their input and the traversal joins the click path's ask. The two mechanisms
  those rounds measured were both the wrong events — a `popstate` guard runs after the router has taken the
  route (and disposes the board), and a page-level `NavigationLock` is not consulted for a traversal —
  while `PlayerFormHistoryTraversalAsksBeforeDiscardingTypedInputAsync` now pins the ask, the refusal and
  the consented departure, and the module's contract case pins the focus fallback that finding 2 of the same
  pass needed. A cross-document traversal is not cancelable in the `navigate` event, so it stays with the
  unload prompt, which is the browser's own.
