...
2. **If `NEEDS_REVISION`**: Triage the findings before routing — some belong to the implementer, some to the test-writer:
   ...
   **Source-code findings** (implementer owns these): BLOCKERs or MAJORs in files under `Nova/`, `Nova.UI/`, `Nova.Client/`, or `Nova.Shared/`:
   ```
   #runSubagent implementer "Fix the following source-code findings for Phase [N].
   ...
   **Test findings** (test-writer owns these): BLOCKERs or MAJORs in files under `Nova.Unit.Tests/` or `Nova.Integration.Tests/`, OR findings that say tests are failing:
   ```
   #runSubagent test-writer "Fix the following test findings for Phase [N].
