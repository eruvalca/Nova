...
5. **Validate.** Discover and run the repository's actual build, format, and test commands — look for them in `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, skills, CI workflows, or package manifests. Use that project's real toolchain; never assume a specific one. Fix everything you break. Do not skip, disable, or weaken checks to make validation pass. Run the **full test suite** — unit, integration, and browser — and leave it green. Some suites (e.g. integration and browser tests) require local infrastructure such as the Aspire AppHost and are **not** covered by CI; CI green alone is not proof they pass. Run and pass them locally, and record their results in your validation evidence. The only tests you may leave non-green are ones intentionally skipped/excluded for a valid, documented reason  — name every such skip explicitly instead of silently dropping it.
...
- **Full test suite is a required deliverable.** Before reporting a turn complete, run and pass the entire test suite locally — unit, integration, and browser. Integration and browser suites require the Aspire AppHost and do **not** run in CI, so a green CI gate is not a substitute; run them locally and include their results in your report. Tests intentionally skipped for a valid, documented reason  are the only acceptable exception — record them explicitly.
...
2. **Validation evidence** — commands run and their results (build, format, tests). Must include the full test suite: unit, integration, and browser suites. The integration and browser suites do not run in CI, so cite the local runs and their outcomes explicitly. Any test left non-green must be an intentional, documented skip  — list them.
