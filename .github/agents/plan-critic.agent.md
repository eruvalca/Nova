...
correctly with zero additional context?"
...
### Axis A — Executability (can a cheap model run each phase with no ambiguity?)

For **every phase**, confirm it contains all of:

- **Exact file paths** for files to create and modify — not "a service" but `Nova/Features/.../X.cs`.
- **Full method/class/interface signatures** to add or change — not "add a method" but the
  complete signature with return type, parameters, and `CancellationToken`.
...
- **No vague verbs** — flag every "implement", "handle", "wire up", "support" that lacks a concrete spec.
...
```
[BLOCKER] `plans/<name>.md` Phase 2 — UserService method has no signature; "add a lookup method" is unexecutable by a low-capability model.
[MAJOR] `plans/<name>.md` Phase 3 — adds an EF migration but no verifier step proving it applies against Postgres.
```

When you can, propose the concrete fix in one clause (e.g., "specify
`Task<ServiceResult<UserDto>> GetUserAsync(long userId, CancellationToken ct)`").
