...
- **No speculative site-wide JS**: do not create an empty `site.js` (or any page-wide script under `Nova/wwwroot/js/`) "just in case" — an empty script still costs a request and invites page-global helpers back. The only site-wide script is Bootstrap's bundle (`<script>` in `App.razor`). A genuinely app-wide behavior belongs in a layout-collocated module (`App.razor.js` / `MainLayout.razor.js`), a lazily imported interop service, or the `blazor.web.js` loader callbacks (`beforeBlazorStarts`) for code that must run before Blazor initializes. Adding a script tag later costs nothing.
