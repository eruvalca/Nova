...
Declare server-side template extensions under **`detector.extensions`** when the project uses Razor, Blade, Twig, ERB, or Handlebars files; the hook skips them otherwise because they sit outside the built-in extension list. Nova configures `{ "ext": ".razor", "engine": "html" }`; `.razor.cs` remains excluded, while isolated CSS and collocated JS retain built-in coverage. `engine` picks the analyzer (`html` for markup templates, `text` for JS/TS/CSS-like files) and defaults to `html`. Match against the end of the filename, so double extensions like `.blade.php` and `.html.erb` work. Config only adds extensions; the built-in list always applies.

Manual `node .github/skills/impeccable/scripts/detect.mjs` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `node .github/skills/impeccable/scripts/detect.mjs --no-config ...` for a raw detector run that ignores project config/context. Use the local hook-admin.mjs commands below to manage the same detector ignores.

Supported harnesses: Claude Code (`.claude/settings.local.json` in the project, which is gitignored so the hook stays machine-local; a hook you move into the shared `settings.json` is honored in place too), Codex (`.codex/hooks.json` in the project), Cursor (`.cursor/hooks.json` in the project), Grok Build (`.grok/hooks/impeccable.json` in the project; requires `/hooks-trust` or `--trust`), and GitHub Copilot (`.github/hooks/impeccable.json` in the project). Copilot CLI discovers the checked-out repository's hooks; cloud execution uses the repository/configuration available in its job. These are separate activation contexts. Do not assume CLI hooks execute during GitHub PR code review.

Codex and Copilot launchers locate their scripts from the Git root and preserve stdin and the event's `cwd`. They set `IMPECCABLE_HOOK_PROJECT_ROOT` for repo-level detector configuration and session caches, including when a nested application has its own `package.json`. Codex supplies a `commandWindows` that explicitly invokes Windows PowerShell; Copilot supplies native `powershell` and `bash` fields. Existing Node 22+ is required by the launchers; no runtime or package is installed automatically. Other project shell commands keep their own runtime requirements.

**Verify execution, not just installation.** In a fresh session, inspect the host's hook diagnostics/trust status, make one reversible UI edit in a disposable copy, and confirm the matching advisory reaches the agent. Exercise both root and nested working directories after launcher changes. A manifest, the admin `status` output, or exit code zero proves neither dispatch nor a successful scan. Codex CLI and desktop require separate execution evidence; respect native trust review and never change personal settings to bypass it. Official references: [Codex hooks](https://learn.chatgpt.com/docs/hooks), [Copilot hooks](https://docs.github.com/en/copilot/reference/hooks-reference).

For manual fallback, run the bundled detector from the repository root with an existing supported runtime: `node .github/skills/impeccable/scripts/detect.mjs <path>`. This avoids `npx` downloading packages. Preserve and inspect stderr: `Scan skipped` is unverified coverage; `DEGRADED` means only the documented fallback analysis ran. Missing optional HTML parser modules leave custom properties, selector matching, and computed contrast unverified even if the regex pass reports no findings. Confirm those properties in the browser when relevant.
...
4. If `<action>` is `on`, follow up with: "The design hook is configured. Verify host trust and confirm an advisory after a UI edit in a fresh session."
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `node .github/skills/impeccable/scripts/detect.mjs <path>` first to see what actually fires there.
...
- The hook is bundled with the Impeccable skill and installed through project-local manifests: `.claude/settings.local.json`, `.codex/hooks.json`, `.cursor/hooks.json`, and `.github/hooks/impeccable.json`. On Codex, inspect `/hooks` and complete any native trust review; changed commands may require renewed review. On Cursor, confirm hooks are enabled under Settings -> Hooks. On Copilot, use fresh-session diagnostics for the checked-out configuration. Cloud execution and GitHub PR review must be verified separately.
...
- Launchers report missing Git roots, hook files, or supported runtimes to stderr and exit without blocking the turn. The adapter similarly records malformed input or detector failures when audit logging is enabled. Treat those paths as skipped or failed validation, never a clean result; use manual fallback and record the limitation.
