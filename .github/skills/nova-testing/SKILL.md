...
description: >-
  ...
  USE FOR: write a unit test, add an integration test, run tests, dotnet test, which test project, tenancy test harness, NovaAppHostFixture, filter tests, MTP flags.
  DO NOT USE FOR: building features (use add-feature-slice) or adding endpoints (use add-api-endpoint).
---
...
6. Run the smallest targeted command with `dotnet test --project <project> --filter-class "*Name"`.
