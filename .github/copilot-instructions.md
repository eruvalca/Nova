...
- OpenID document: `/openapi` (Development only).
...
- `.github/instructions/bootstrap-theme.instructions.md` for the Sass-compiled kelp-forest Bootstrap theme: `Nova/scss/_variables.scss` is the single source of truth, `npm run build:css` / `npm run check:contrast` are authoritative, Node 20+/npm only, the compiled `Nova/wwwroot/css/bootstrap-theme.css` is generated (never edit or commit it), and never re-add vendored Bootstrap CSS or Bootstrap-blue literals.
